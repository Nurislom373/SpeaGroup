...
public class AuthFollowerController extends AbstractController<AuthFollowerService> {
    ...
    public ResponseEntity<Data<String>> add(@RequestBody AuthFollowerCreateDTO dto) {
        service.create(dto);
    ...
    public ResponseEntity<Data<String>> delete(@PathVariable String id) {
        service.delete(id);
        return new ResponseEntity<>(new Data<>("Successfully Deleted - Auth Follower"), HttpStatus.OK);
    }
    ...
    public ResponseEntity<Data<List<AuthFollowerGetDTO>>> get(@Valid @NotNull AuthFollowerCriteria criteria) {
